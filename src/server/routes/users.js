...
router.get('/:userId/votes', passport.authenticate('jwt', { session: false }), (req, res) => {
  db.any(`
        SELECT 
          post_id,
...
router.get('/:userId/hiddenPosts', passport.authenticate('jwt', { session: false }), (req, res) => {
  db.any(`
        SELECT 
          post_id
        FROM 
...
router.get('/:userId/savedPosts', passport.authenticate('jwt', { session: false }), (req, res) => {
  db.any(`
        SELECT 
          post_id
        FROM 
